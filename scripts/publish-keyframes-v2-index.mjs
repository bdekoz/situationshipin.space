#!/usr/bin/env node
// Publish the deterministic per-location keyframes as a review family on
// situationshipin.space (generation-keyframes-20260818-v2).
//
// Usage:
//   node scripts/publish-keyframes-v2-index.mjs \
//     --izzi-commit <40-hex> --artifacts-dir <dir with keyframe-*.png>
...
const FAMILY = "generation-keyframes-20260818-v2";
const ARTIFACT_ID = "generation-keyframes-20260818-v2-index";
const MEDIA_DIR = "review/media/keyframes-v2";

const MEMBERS = [
  { name: "keyframe-frogtown", title: "Frogtown keyframe", description: "Deterministic neon still framed from the Frogtown corridor route." },
  { name: "keyframe-dtla-central-library", title: "DTLA Central Library keyframe", description: "Deterministic neon still framed from the Central Library route." },
  { name: "keyframe-moca-geffen", title: "MOCA Geffen keyframe", description: "Deterministic neon still framed from the MOCA Geffen route." },
  { name: "keyframe-hauser-wirth", title: "Hauser & Wirth keyframe", description: "Deterministic neon still framed from the Hauser & Wirth route." },
  { name: "keyframe-hammer-museum", title: "Hammer Museum keyframe", description: "Deterministic neon still framed from the Hammer Museum route." },
  { name: "keyframe-hiho-to-runyon-canyon", title: "Hi-Ho to Runyon keyframe", description: "Deterministic neon still framed from the Hi-Ho to Runyon route." },
  { name: "keyframe-runyon-mulholland-drive", title: "Runyon Mulholland keyframe", description: "Deterministic neon still framed from the Runyon Mulholland route." },
  { name: "keyframe-runyon-to-inspiration-point", title: "Inspiration Point keyframe", description: "Deterministic neon still framed from the Inspiration Point route." },
];
...
async function main() {
  ...
  if (!izziCommit || !/^[0-9a-f]{40}$/.test(izziCommit) || !artifactsDir) {
    console.error("usage: node scripts/publish-keyframes-v2-index.mjs --izzi-commit <40-hex> --artifacts-dir <dir>");
    process.exitCode = 1;
    return;
  }
  ...
  catalog.items = catalog.items.filter((item) =>
    !/^keyframe-/.test(item.artifact_id || ""));
  for (const spec of MEMBERS) {
    ...
    const item = {
      ...
      alt: `${spec.title}, 360 by 360 deterministic neon still.`,
      family: FAMILY,
      generation_class: "keyframes",
      feedback_round: FAMILY,
      media_kind: "image",
      review_scope: "GENERATION-KEYFRAMES-20260818-V2",
      review_mode: "output",
      source_path: `izzi ${izziCommit} outputs/ad-hoc/keyframes.v1/svg/${spec.name}.svg`,
  ...
  const title = "Keyframes Capability 20260818 v2";
  const description = "Deterministic per-location neon keyframes for the fal.ai video path: eight independently authored stills framed from the here-lies-trouble routes.";
  ...
  const entry = {
    ...
    title, description,
    alt: "Index of the 8 deterministic izzi keyframe review plates from 2026-08-18.",
    family: FAMILY,
    generation_class: "keyframes-index",
    feedback_round: FAMILY,
    media_kind: "index",
    review_scope: "GENERATION-KEYFRAMES-20260818-V2",
    review_mode: "output",
    source_path: `izzi ${izziCommit} outputs/ad-hoc/keyframes.v1 (per-location stills)`,
    ...
    technical_state: "KEYFRAMES-INDEX",
    human_review_state: "UNREVIEWED",
    baseline_state: "NOT-PROMOTED",
    review_category: "proofs",
    added_at: new Date().toISOString(),
