#!/usr/bin/env node
// Publish the revised izzi Dutch lighthouse radiate guilloche review plate as
// a single-member review family on situationshipin.space
// (generation-guilloche-20260821-dutch-v2), per review issue #59.
//
// Usage:
//   node scripts/publish-guilloche-dutch-v2-index.mjs \
...
const FAMILY = "generation-guilloche-20260821-dutch-v2";
const ARTIFACT_ID = "generation-guilloche-20260821-dutch-v2-index";
const MEDIA_DIR = "review/media/guilloche-dutch";
const SOURCE_SVG = "dutch-plate-grid-3x5.svg";

const MEMBER = {
  name: "dutch-plate-grid-3x5-v2",
  title: "3x5 review plate grid v2",
  description: "The full 4800x2880 Dutch lighthouse radiate review plate at 2x scale: rows black/white reference, exploration, and wild treatments across stroke widths 0.5, 1, 2, 4, and 8. Single grid only; per-cell plates are not generated.",
...
async function main() {
  ...
  if (!izziCommit || !/^[0-9a-f]{40}$/.test(izziCommit) || !artifactsDir) {
    console.error("usage: node scripts/publish-guilloche-dutch-v2-index.mjs --izzi-commit <40-hex> --artifacts-dir <dir>");
    process.exitCode = 1;
    return;
  }
  ...
  // Mark the superseded 20260821 dutch family STALE with the REVISE decision
  // from review issue #59. The stale family stays in the catalog so its pages
  // and media remain referenced, but the proofs-for-inspection surface hides
  // STALE proofs.
  for (const item of catalog.items) {
    if (item.family === "generation-guilloche-20260821-dutch") {
  ...
  const memberItem = {
    ...
    media_kind: "image",
    review_scope: "GENERATION-GUILLOCHE-20260821-DUTCH-V2",
    review_mode: "output",
  ...
  const title = "Guilloche Dutch Lighthouse Radiate 20260821 v2";
  const description = "Stage-4 Dutch guilloche review, revision 2 (issue #59): the sol-5.6 reference-guided two-layer lighthouse radiate as one 2x 3x5 plate - row 1 black/white reference, row 2 seeded randoma11y color-pair explorations, row 3 wild glitch/extreme treatments - across stroke widths 0.5, 1, 2, 4, and 8. Per-cell plates are not generated. Prior round: generation-guilloche-20260821-dutch-index (STALE).";
  ...
  const entry = {
    ...
    title, description,
    alt: "Index of the single revised izzi Dutch lighthouse radiate review plate from 2026-08-21 (issue #59).",
    family: FAMILY,
    ...
    review_scope: "GENERATION-GUILLOCHE-20260821-DUTCH-V2",
    review_mode: "output",
    source_path: `izzi ${izziCommit} outputs/ad-hoc/guilloche.dutch/svg/${SOURCE_SVG} (revised 3x5 plate)`,
    ...
    technical_state: "GUILLOCHE-DUTCH-V2-INDEX",
    human_review_state: "UNREVIEWED",
    baseline_state: "NOT-PROMOTED",
    review_category: "proofs",
    added_at: new Date().toISOString(),
